...
const FALLBACK_ZH_PHRASE = '我已阅读、理解并同意 ModuRelay 部署与运营合规承诺'
const FALLBACK_EN_PHRASE = 'I have read, understood, and agree to the ModuRelay Deployment and Operation Compliance Commitment'
...
export const useAdminComplianceStore = defineStore('adminCompliance', () => {
  ...
  function requireAcknowledgement(partialStatus?: Partial<AdminComplianceStatus>): void {
    status.value = {
      ...
      document_url_zh: partialStatus?.document_url_zh || status.value?.document_url_zh || 'https://github.com/lien0219/modurelay/blob/main/docs/legal/admin-compliance.zh.md',
      document_url_en: partialStatus?.document_url_en || status.value?.document_url_en || 'https://github.com/lien0219/modurelay/blob/main/docs/legal/admin-compliance.en.md',

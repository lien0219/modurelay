export default {
    accounts: {
      ...
      healthScore: {
        hint: 'Unified health score from 0 to 100. Higher scores are preferred only among schedulable accounts in the same group and priority tier; rate limits, quota, model capability, and channel pricing still take precedence.',
        states: {
          warming: 'Warming',
          healthy: 'Healthy',
          degraded: 'Degraded',
        ...
        latency: 'EWMA latency: {value} ms',
        lastFailure: 'Latest failure: {reason}',
        retryAt: 'Recovery probe at: {time}'
      },
      usageWindowsHint: '"5h / 7d" are the upstream account\'s official rolling usage windows (e.g. OpenAI ChatGPT, Claude). They are imposed by the upstream provider on the account itself — not configured by ModuRelay, and unrelated to the models you map. Usage resets automatically once each window rolls over, and the limit cannot be lifted from within ModuRelay.',
      ollamaCloud: {
      ...
      upstreamBilling: {
        trustWarning: 'This rate is declared by the upstream site for the current API key. ModuRelay cannot verify that it matches actual charges. The upstream site or an intermediary may return forged, stale, or modified data. Verify it against bills, balance changes, and actual usage.',
        autoProbe: 'Automatically probe upstream rate and balance',
        autoProbeHint: 'Refresh the upstream declared rate and available balance on the global interval. A successful, valid zero-or-negative balance pauses scheduling; re-enable scheduling manually after funding.',
        syncRate: 'Sync upstream declared rate',
        syncRateHint: 'Update the account rate after each successful probe, using the base rate excluding peak hours. Failed probes or declarations outside the allowed range leave it unchanged. Enabling this also turns on "Automatically probe upstream rate and balance".',
        ...
        newAPIGroup: 'New API upstream group',
        newAPIGroupPlaceholder: 'Select the upstream group used for rate sync (optional)',
        newAPIGroupHint: 'The list comes from New API public pricing. A group controls the billing rate and has no separate balance; select the group actually used by this token. This synchronizes its rate without modifying the upstream token. Leaving it empty still probes token quota or wallet balance.',
        newAPIGroupEmpty: 'The upstream returned no publicly selectable groups',
        newAPIGroupsProbeFailed: 'New API was identified, but its group list could not be queried. Refresh upstream information before selecting a group.',
        newAPIGroupUnavailable: 'The selected group is no longer in the upstream list. Select another group or clear it.',
        newAPIGroupUnavailableOption: '{name} (no longer available upstream)',
        newAPIGroupNotSelected: 'No New API upstream group selected (optional)',
        newAPIGroupProbeFailed: 'New API group rate query failed',
        newAPIWalletProbe: 'Probe New API account wallet',
        newAPIWalletProbeHint: 'When the relay token has unlimited quota, use a separate user access token to read the account wallet. ModuRelay first verifies that the account owns the relay token; only a confirmed zero-or-negative wallet balance pauses scheduling.',
        newAPIUserAccessToken: 'New API user access token (PAT)',
        newAPIUserAccessTokenPlaceholder: 'Enter a persistent New API user access token',
        newAPIUserAccessTokenHint: 'Generate a persistent access token in New API personal settings. It must belong to the same account as this API key; do not use a short-lived dashboard login JWT. The server stores it only as ciphertext and requires a fixed TOTP_ENCRYPTION_KEY.',
        newAPIUserAccessTokenConfiguredHint: 'An access token is configured. Leave this blank to keep it, or turn off the switch above to remove it. The token must belong to the same New API account as this API key.',
        newAPIUserAccessTokenRequired: 'A New API user access token is required when account wallet probing is enabled.',
        newAPIUserID: 'New API user ID (legacy compatibility, optional)',
        newAPIUserIDPlaceholder: 'For example: 123',
        newAPIUserIDHint: 'Leave this blank for current New API releases. If an older release reports a missing New-Api-User header, enter the numeric user ID from the profile page; ModuRelay will retry with the legacy authentication format.',
        newAPIUserIDInvalid: 'The user ID must be an integer from 1 to 2147483647.',
        manualProbe: 'Probe upstream rate and balance now',
        stale: 'Stale',
        ...
        enabled: 'On',
        disabled: 'Off',
        probeFailed: 'Failed to probe upstream information',
        probeUnsupported: 'The upstream does not support rate or balance probing',
        probeCompleted: 'Upstream balance and rate refresh completed',
        noEligibleAccounts: 'Select API key accounts',
        batchLimit: 'A batch can probe at most 20 accounts',
        batchCompleted: 'Probed upstream information for {count} account(s)',
        batchPartial: 'Upstream information probe partially completed: {success} succeeded, {failed} failed'
      },
      upstreamBalance: {
        trustWarning: 'Balance and remaining quota are reported by the upstream for this API key. ModuRelay only acts on a successful, valid zero-or-negative observation: it marks the account unschedulable; failed, stale, unsupported, invalid, and unlimited responses do not change scheduling. Re-enable scheduling manually after the upstream is funded.',
        details: 'Upstream balance details: {value}',
        manualProbe: 'Refresh upstream balance and rate',
        refreshing: 'Refreshing upstream balance and rate',
        stale: 'Stale',
        unsupported: 'Unsupported',
        failed: 'Query failed',
        invalid: 'Key unavailable',
        exhausted: 'Exhausted',
        exhaustedAutoPaused: 'Exhausted; scheduling paused',
        autoPaused: 'Scheduling remains paused',
        notProbed: 'Not probed',
        unlimited: 'Unlimited',
        tokenUnlimited: 'Token unlimited',
        walletNotConfigured: 'Wallet not configured',
        walletProbeFailed: 'Wallet query failed',
        walletNotConfiguredDetail: 'No New API user access token is configured. The relay token is confirmed unlimited, but the account wallet cannot be read.',
        walletProbeError: 'Account wallet probe failed: {value}',
        mode: 'Funding mode: {value}',
        walletBalance: 'Wallet balance: {value}',
        remaining: 'Remaining: {value}',
        usedLimit: 'Used / limit: {value}',
        updatedAt: 'Updated: {value}',
        source: 'Source: {value}',
        nextProbeAt: 'Next probe: {value}',
        modes: {
        ...
        sources: {
          billing: 'Upstream billing declaration',
          usage: '/v1/usage',
          newAPIToken: 'New API token quota',
          newAPIWallet: 'New API account wallet'
        },
        errors: {
          request_build_failed: 'Could not create the balance request',
          request_failed: 'Could not reach the upstream',
          request_timeout: 'The upstream connection timed out',
          dns_failed: 'The upstream hostname could not be resolved',
          resolved_address_blocked: 'The resolved upstream address was blocked by policy',
          proxy_failed: 'The proxy could not connect to the upstream',
          protocol_failed: 'The upstream HTTP protocol negotiation failed',
          tls_failed: 'The upstream TLS connection failed',
          connection_failed: 'Could not establish an upstream connection',
          empty_response: 'The upstream returned an empty response',
          response_read_failed: 'Could not read the upstream response',
          response_too_large: 'The upstream response exceeded the size limit',
          unauthorized: 'The upstream rejected this key',
          http_error: 'The upstream returned an error',
          invalid_response: 'The upstream balance format was not recognized',
          stale_response: 'The upstream returned an outdated observation',
          ownership_invalid_response: 'The New API token ownership response was not recognized',
          token_owner_mismatch: 'The user access token and relay token do not belong to the same New API account',
          token_ownership_unverified: 'Could not safely verify that this account owns the relay token',
          user_mismatch: 'The configured New API user ID does not match the access token account',
          credential_reentry_required: 'This access token was stored as plaintext by an earlier version. Re-enter it in account settings to complete encrypted migration.',
          encryption_key_unavailable: 'A fixed TOTP_ENCRYPTION_KEY is not configured, so the access token cannot be decrypted',
          credential_decrypt_failed: 'The access token could not be decrypted. Check TOTP_ENCRYPTION_KEY or re-enter the token.',
          credential_invalid: 'The decrypted New API access token is invalid'
        }
      },
      ...
      bulkActions: {
        ...
        refreshToken: 'Refresh Token',
        probeUpstreamBilling: 'Probe Upstream Rate & Balance',
      ...
      openai: {
        ...
        endpointCapabilitiesDesc:
          'Used by account routing. Text follows the Responses API setting above; Embeddings controls /v1/embeddings; Videos enables the OpenAI-compatible /v1/videos adapter. Grok media keeps its dedicated path.',
      ...
      poolModeInfo:
        'When enabled, upstream 429/403/401 errors will auto-retry without marking the account as rate-limited or errored. Suitable for upstream pointing to another ModuRelay instance.',

export default {
    accounts: {
      ...
      healthScore: {
        hint: '统一健康分范围为 0～100。仅在同一分组、同一优先级的可调度账号中优先选择高分账号；限流、配额、模型能力和渠道定价规则仍优先生效。',
        states: {
          warming: '采样中',
          healthy: '健康',
          degraded: '降级',
        ...
        latency: 'EWMA 延迟：{value} ms',
        lastFailure: '最近失败：{reason}',
        retryAt: '恢复探测时间：{time}'
      },
      usageWindowsHint: '“5h / 7d”是上游账号（如 OpenAI ChatGPT、Claude）官方的滚动用量窗口限制，由上游对账号设定，并非 ModuRelay 配置，也与你映射的模型无关。窗口滚动到期后用量会自动重置，无法在 ModuRelay 端解除该限制。',
      ollamaCloud: {
      ...
      upstreamBilling: {
        trustWarning: '此倍率由上游站点针对当前 API Key 自行声明。ModuRelay 无法验证该值是否与实际扣费一致；上游站点或中间代理可能返回伪造、过期或被篡改的数据。请结合账单、余额变化和实际用量自行核验。',
        autoProbe: '自动探测上游倍率与余额',
        autoProbeHint: '启用后按全局周期刷新上游声明倍率和可用余额；只有成功且有效的余额为 0 或负数时会自动停止调度，恢复调度需管理员手动开启。',
        syncRate: '同步上游声明倍率',
        syncRateHint: '成功探测后自动更新账号倍率，同步的是不含高峰的基准倍率；探测失败或声明超出允许范围时保持不变。开启本项会同时打开“自动探测上游倍率与余额”。',
        syncRateManagedHint: '当前倍率由上游声明的基准倍率（不含高峰）自动维护。',
        syncedRateTooltip: '该账号倍率由上游声明的基准倍率（不含高峰）自动同步',
        newAPIGroup: 'New API 上游分组',
        newAPIGroupPlaceholder: '选择用于倍率同步的上游分组（可选）',
        newAPIGroupHint: '列表来自 New API 公开定价。分组只决定计费倍率，不存在独立分组余额；请选择该令牌实际使用的分组。这里只同步倍率，不会修改上游令牌。留空时仍会探测令牌额度或钱包余额。',
        newAPIGroupEmpty: '上游未返回公开可选分组',
        newAPIGroupsProbeFailed: 'New API 已识别，但分组列表查询失败。请先刷新上游信息后再选择。',
        newAPIGroupUnavailable: '当前选择已不在上游返回的分组列表中，请重新选择或清空。',
        newAPIGroupUnavailableOption: '{name}（上游已不可用）',
        newAPIGroupNotSelected: '未选择 New API 上游分组（可选）',
        newAPIGroupProbeFailed: 'New API 分组倍率查询失败',
        newAPIWalletProbe: '探测 New API 账户钱包',
        newAPIWalletProbeHint: '仅在当前转发令牌为无限额度时，使用独立的用户访问令牌读取账户钱包；系统会先核验该账户确实持有当前转发令牌，确认钱包余额为 0 或负数后才会自动停止调度。',
        newAPIUserAccessToken: 'New API 用户访问令牌（PAT）',
        newAPIUserAccessTokenPlaceholder: '输入 New API 长期用户访问令牌',
        newAPIUserAccessTokenHint: '请在 New API 个人设置中生成长期访问令牌。它必须与当前 API Key 属于同一账户；不要填写网页登录产生的短期 JWT。服务端仅以密文保存，需预先配置固定 TOTP_ENCRYPTION_KEY。',
        newAPIUserAccessTokenConfiguredHint: '已配置访问令牌。留空会保留现有值；关闭上方开关会删除。令牌必须与当前 API Key 属于同一 New API 账户。',
        newAPIUserAccessTokenRequired: '启用账户钱包探测时必须填写 New API 用户访问令牌。',
        newAPIUserID: 'New API 用户 ID（旧版兼容，可选）',
        newAPIUserIDPlaceholder: '例如：123',
        newAPIUserIDHint: '新版 New API 可留空。旧版若提示未提供 New-Api-User，请填写个人资料中的数字用户 ID，系统会自动改用旧版认证格式重试。',
        newAPIUserIDInvalid: '用户 ID 必须是 1 到 2147483647 之间的整数。',
        manualProbe: '立即探测上游倍率与余额',
        stale: '已过期',
        unsupported: '不支持',
        failed: '失败',
        notProbed: '未探测',
        ...
        enabled: '打开',
        disabled: '关闭',
        probeFailed: '探测上游信息失败',
        probeUnsupported: '上游不支持倍率或余额探测',
        probeCompleted: '上游余额与倍率刷新完成',
        noEligibleAccounts: '请选择 API Key 账号',
        batchLimit: '每次最多探测 20 个账号',
        batchCompleted: '已完成 {count} 个账号的上游信息探测',
        batchPartial: '上游信息探测部分完成：成功 {success} 个，失败 {failed} 个'
      },
      upstreamBalance: {
        trustWarning: '余额和剩余额度由上游针对当前 API Key 返回。ModuRelay 仅在成功且有效的余额明确为 0 或负数时自动停止该账号调度；失败、过期、不支持、无效和无限额度响应不会改变调度。上游充值后请手动重新开启调度。',
        details: '上游余额详情：{value}',
        manualProbe: '立即刷新上游余额与倍率',
        refreshing: '正在刷新上游余额与倍率',
        stale: '已过期',
        unsupported: '不支持',
        failed: '查询失败',
        invalid: 'Key 不可用',
        exhausted: '已用尽',
        exhaustedAutoPaused: '已用尽，已自动停调',
        autoPaused: '仍处于自动停调',
        notProbed: '未探测',
        unlimited: '无限制',
        tokenUnlimited: '令牌无限',
        walletNotConfigured: '钱包未配置',
        walletProbeFailed: '钱包查询失败',
        walletNotConfiguredDetail: '未配置 New API 用户访问令牌，当前只能确认转发令牌为无限额度，无法读取账户钱包。',
        walletProbeError: '账户钱包探测失败：{value}',
        mode: '资金模式：{value}',
        walletBalance: '钱包余额：{value}',
        remaining: '剩余额度：{value}',
        usedLimit: '已用 / 上限：{value}',
        updatedAt: '更新时间：{value}',
        source: '数据来源：{value}',
        nextProbeAt: '下一次探测：{value}',
        modes: {
        ...
        sources: {
          billing: '上游计费自描述',
          usage: '/v1/usage',
          newAPIToken: 'New API 令牌额度',
          newAPIWallet: 'New API 账户钱包'
        },
        errors: {
          request_build_failed: '无法创建余额请求',
          request_failed: '无法连接上游',
          request_timeout: '连接上游超时',
          dns_failed: '上游域名解析失败',
          resolved_address_blocked: '上游解析地址被安全策略拦截',
          proxy_failed: '代理连接上游失败',
          protocol_failed: '上游 HTTP 协议协商失败',
          tls_failed: '上游 TLS 连接失败',
          connection_failed: '无法建立上游连接',
          empty_response: '上游响应为空',
          response_read_failed: '读取上游响应失败',
          response_too_large: '上游响应超过大小限制',
          unauthorized: '上游拒绝当前 Key',
          http_error: '上游返回错误',
          invalid_response: '无法识别上游余额格式',
          ownership_invalid_response: '无法识别 New API 令牌归属响应',
          token_owner_mismatch: '当前用户访问令牌与转发令牌不属于同一 New API 账户',
          token_ownership_unverified: '无法安全核验当前账户是否持有该转发令牌',
          user_mismatch: '配置的 New API 用户 ID 与访问令牌账户不一致',
          credential_reentry_required: '该访问令牌由旧版本明文保存，请在账号编辑中重新输入以完成加密迁移',
          encryption_key_unavailable: '服务未配置固定 TOTP_ENCRYPTION_KEY，无法解密访问令牌',
          credential_decrypt_failed: '访问令牌解密失败，请检查 TOTP_ENCRYPTION_KEY 或重新输入令牌',
          credential_invalid: '解密后的访问令牌格式无效，请重新输入',
          stale_response: '上游返回了过期的观测数据'
        }
      },
      ...
      usageWindow: {
        ...
        grokRequests: '请求',
        grokTokens: 'Token',
        grokFreeQuota24hHint: '按 ModuRelay 近 24 小时本地 Token 用量估算（上限 {limit}）',
      ...
      bulkActions: {
        ...
        resetStatus: '批量重置状态',
        refreshToken: '批量刷新令牌',
        probeUpstreamBilling: '探测上游倍率与余额',
      ...
      openai: {
        ...
        endpointCapabilitiesDesc:
          '用于调度筛选。文本端点跟随上方 Responses API 设置；Embeddings 控制 /v1/embeddings；Videos 启用 OpenAI 兼容的 /v1/videos 适配。Grok 媒体仍保留专用链路。',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 ModuRelay 实例的场景。',

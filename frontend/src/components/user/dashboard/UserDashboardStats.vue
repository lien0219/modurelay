<template>
  <!-- Row 1: Core Stats -->
  <div data-reveal class="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
    <!-- Balance -->
    <div v-if="!isSimple" class="card glass-card-accent min-w-0 p-4">
      <div class="relative z-[1] flex items-center gap-3">
        ...
        <div class="min-w-0">
          ...
          <p class="technical-value kpi-value text-xl font-bold text-emerald-600 dark:text-emerald-400">
    ...
    <!-- API Keys -->
    <div class="card glass-card-accent min-w-0 p-4">
      <div class="relative z-[1] flex items-center gap-3">
        ...
        <div class="min-w-0">
          ...
          <p class="technical-value kpi-value text-xl font-bold text-gray-900 dark:text-white">
    ...
    <div class="card glass-card-accent min-w-0 p-4">
      <div class="relative z-[1] flex items-center gap-3">
        <div class="rounded-lg bg-secondary-100 p-2 dark:bg-secondary-900/30">
          <Icon name="chart" size="md" class="text-secondary-600 dark:text-secondary-400" :stroke-width="2" />
        </div>
        <div class="min-w-0">
  ...
  <div data-reveal class="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
    ...
    <div class="card min-w-0 p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-secondary-100 p-2 dark:bg-secondary-900/30">
          <Icon name="cube" size="md" class="text-secondary-600 dark:text-secondary-400" :stroke-width="2" />
        </div>
        <div class="min-w-0">
          <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.todayTokens') }}</p>
          <p class="technical-value text-xl font-bold text-gray-900 dark:text-white">
            <AnimatedNumber :value="stats?.today_tokens ?? 0" :format="formatTokens" fallback="0" />
          </p>
          <p class="text-xs leading-5 text-gray-500 [overflow-wrap:anywhere] dark:text-gray-400">{{ t('dashboard.input') }}: {{ formatTokens(stats?.today_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.today_output_tokens || 0) }} / {{ t('dashboard.cache') }}: {{ formatTokens((stats?.today_cache_creation_tokens || 0) + (stats?.today_cache_read_tokens || 0)) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card min-w-0 p-4">
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0">
          ...
          <p class="text-xs leading-5 text-gray-500 [overflow-wrap:anywhere] dark:text-gray-400">{{ t('dashboard.input') }}: {{ formatTokens(stats?.total_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.total_output_tokens || 0) }} / {{ t('dashboard.cache') }}: {{ formatTokens((stats?.total_cache_creation_tokens || 0) + (stats?.total_cache_read_tokens || 0)) }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="card min-w-0 p-4">
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
          ...
          <div class="flex items-baseline gap-2">
            <p class="technical-value text-sm font-semibold text-primary-600 dark:text-primary-400">
    ...
    <div class="card min-w-0 p-4">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-secondary-100 p-2 dark:bg-secondary-900/30">
          <Icon name="clock" size="md" class="text-secondary-600 dark:text-secondary-400" :stroke-width="2" />
        </div>
        <div class="min-w-0">

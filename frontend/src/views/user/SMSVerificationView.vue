...
async function resend(id: string) {
  try { await smsAPI.resend(id); await refreshOrder(id); appStore.showSuccess('已清除当前短信，可继续接收新的验证码') }
  catch (error) { appStore.showError(errorMessage(error, '再次接码失败')) }
}

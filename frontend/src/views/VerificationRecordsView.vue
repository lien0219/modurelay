          ...
          <table class="min-w-[3300px] text-left text-sm">
            ...
            <tbody>
              ...
              <tr v-for="record in records" :key="record.id" class="border-t border-gray-100 align-top dark:border-dark-700">
                ...
                <td class="px-4 py-3"><div class="min-w-44">{{ record.channel_name }}</div><div class="font-mono text-xs text-gray-500">{{ record.channel_code }}</div></td><td class="px-4 py-3 font-mono text-xs"><div class="min-w-52 break-all">{{ record.target || '-' }}</div></td><td class="whitespace-nowrap px-4 py-3 font-mono text-xs">{{ record.provider_code || '-' }}</td><td class="px-4 py-3"><span class="badge whitespace-nowrap" :class="outcomeClass(record.outcome)">{{ outcomeLabel(record.outcome) }}</span><div class="mt-1 whitespace-nowrap font-mono text-xs text-gray-500">{{ record.status }}</div></td><td class="px-4 py-3"><div class="whitespace-nowrap">{{ record.refund_status }}</div><div v-if="record.refund_reason" class="mt-1 max-w-52 text-xs text-gray-500">{{ record.refund_reason }}</div></td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.sale_amount, record.currency) }}</td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.provider_cost, record.currency) }}</td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.user_debit_amount, record.currency) }}</td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.reserved_amount, record.currency) }}</td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.captured_amount, record.currency) }}</td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.released_amount, record.currency) }}</td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.refunded_amount, record.currency) }}</td><td class="px-4 py-3 text-center tabular-nums">{{ record.provider_request_count }}</td><td class="px-4 py-3"><div class="min-w-72 font-mono text-xs text-red-600 dark:text-red-300">{{ record.error_code || '-' }}</div><div v-if="record.error_message" class="mt-1 max-w-96 whitespace-normal break-words text-xs text-gray-500 dark:text-gray-400">{{ record.error_message }}</div></td>
              </tr>
            </tbody>
          </table>
        </div>

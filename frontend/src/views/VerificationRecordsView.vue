          ...
          <table class="min-w-[3300px] whitespace-nowrap text-left text-sm">
            ...
            <tbody>
              ...
              <tr v-for="record in records" :key="record.id" class="border-t border-gray-100 align-top dark:border-dark-700">
                ...
                <td class="whitespace-nowrap px-4 py-3 text-xs text-gray-500 dark:text-gray-400">{{ formatDate(record.created_at) }}</td><td class="whitespace-nowrap px-4 py-3"><span class="badge badge-info">{{ typeLabel(record.verification_type) }}</span></td><td class="px-4 py-3"><div class="min-w-52 font-medium text-gray-900 dark:text-white">{{ record.user_email || '-' }}</div><div class="text-xs text-gray-500">ID {{ record.user_id }}</div></td><td class="px-4 py-3 text-xs"><CopyableIdentifier :value="record.order_no" max-width="15rem" /></td>
                ...
                <td class="px-4 py-3"><div class="min-w-44">{{ isAdmin ? record.channel_name : publicChannelLabel(record) }}</div><div v-if="isAdmin" class="font-mono text-xs text-gray-500">{{ record.channel_code }}</div></td><td class="px-4 py-3 font-mono text-xs"><div class="min-w-52 break-all">{{ record.target || '-' }}</div></td><td class="whitespace-nowrap px-4 py-3 font-mono text-xs">{{ record.provider_code || '-' }}</td><td class="px-4 py-3"><span class="badge whitespace-nowrap" :class="outcomeClass(record.outcome)">{{ outcomeLabel(record.outcome) }}</span><div class="mt-1 whitespace-nowrap font-mono text-xs text-gray-500">{{ record.status }}</div></td><td class="px-4 py-3"><div class="block max-w-52 truncate" :title="refundStatusLabel(record.refund_status)">{{ refundStatusLabel(record.refund_status) }}</div><div v-if="record.refund_reason" class="mt-1 block max-w-52 truncate text-xs text-gray-500" :title="refundReasonLabel(record.refund_reason)">{{ refundReasonLabel(record.refund_reason) }}</div></td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.sale_amount, record.currency) }}</td><td class="whitespace-nowrap px-4 py-3 tabular-nums"><span>{{ money(record.provider_cost, record.currency) }}</span><span v-if="record.provider_cost_estimated" class="ml-1 text-[10px] text-amber-600 dark:text-amber-300">{{ t('verificationRecords.estimated') }}</span></td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.user_debit_amount, record.currency) }}</td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.reserved_amount, record.currency) }}</td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.captured_amount, record.currency) }}</td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.released_amount, record.currency) }}</td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.refunded_amount, record.currency) }}</td><td class="px-4 py-3 text-center tabular-nums">{{ record.provider_request_count ?? '-' }}</td><td class="px-4 py-3"><div class="min-w-72 font-mono text-xs text-red-600 dark:text-red-300">{{ record.error_code || '-' }}</div><div v-if="record.error_message" class="mt-1 max-w-96 whitespace-normal break-words text-xs text-gray-500 dark:text-gray-400">{{ record.error_message }}</div></td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div v-else class="w-full min-w-0 max-w-full overflow-x-auto overscroll-x-contain focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary-500" tabindex="0" :aria-label="t('verificationRecords.userTableLabel')">
          <table class="w-full min-w-[1750px] whitespace-nowrap text-left text-sm">
            ...
            <tbody>
              ...
              <tr v-for="record in records" :key="record.id" class="border-t border-gray-100 dark:border-dark-700"><td class="whitespace-nowrap px-4 py-3 text-xs text-gray-500 dark:text-gray-400">{{ formatDate(record.created_at) }}</td><td class="whitespace-nowrap px-4 py-3"><span class="badge badge-info">{{ typeLabel(record.verification_type) }}</span></td><td class="px-4 py-3 text-xs"><CopyableIdentifier :value="record.order_no" max-width="15rem" /></td><td class="px-4 py-3"><VerificationIdentity kind="platform" :code="record.service_code" :label="platformLabel(record.service_code)" /></td><td class="px-4 py-3"><VerificationIdentity v-if="record.verification_type === 'sms' && record.region" kind="country" :code="record.region" :label="countryLabel(record.region)" /><span v-else class="text-gray-400">-</span></td><td class="px-4 py-3"><div class="min-w-44">{{ isAdmin ? record.channel_name : publicChannelLabel(record) }}</div><div v-if="isAdmin" class="font-mono text-xs text-gray-500">{{ record.channel_code }}</div></td><td class="px-4 py-3 font-mono text-xs"><div class="min-w-52 break-all">{{ record.target || '-' }}</div></td><td class="px-4 py-3"><span class="badge whitespace-nowrap" :class="outcomeClass(record.outcome)">{{ outcomeLabel(record.outcome) }}</span></td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.sale_amount, record.currency) }}</td><td class="whitespace-nowrap px-4 py-3 tabular-nums">{{ money(record.refunded_amount, record.currency) }}</td><td class="px-4 py-3"><div class="max-w-64 whitespace-normal break-words text-xs text-gray-500 dark:text-gray-400">{{ record.public_error_message || '-' }}</div></td></tr>
            </tbody>
          </table>
        </div>
...
function publicChannelLabel(record: VerificationRecord): string {
  const code = String(record.channel_code || '').trim().toLowerCase()
  if (record.verification_type === 'email') {
    if (code === 'email_channel_1') return t('email.user.channel1')
    if (code === 'email_channel_2') return t('email.user.channel2')
    return t('email.user.channel')
  }
  const match = /^channel_(\d+)$/.exec(code)

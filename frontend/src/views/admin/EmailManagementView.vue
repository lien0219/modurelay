...
async function saveProvider(provider: EmailProviderAdmin) {
  ...
  try {
    await adminEmail.updateProvider(provider.id, { enabled: provider.enabled, base_url: provider.base_url, credential_ref: credentialRefs.value[provider.id] || undefined, billing: billingDrafts.value[provider.id] })

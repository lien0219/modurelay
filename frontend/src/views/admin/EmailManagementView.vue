<template>
  <AppLayout>
    <div class="mx-auto w-full min-w-0 max-w-[1800px] space-y-5">
      ...
      <section class="card min-w-0 overflow-hidden">
        ...
        <div class="max-w-full overflow-x-auto overscroll-x-contain focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary-500" tabindex="0" :aria-label="t('email.admin.providers')"><table class="w-full min-w-[1400px] text-left text-sm"><thead class="whitespace-nowrap bg-gray-50 text-xs uppercase text-gray-500 dark:bg-dark-800"><tr><th class="px-5 py-3">{{ t('email.admin.name') }}</th><th class="px-5 py-3">{{ t('email.admin.baseUrl') }}</th><th class="px-5 py-3">{{ t('email.admin.credential') }}</th><th class="px-5 py-3">{{ t('email.admin.health') }}</th><th class="px-5 py-3">{{ t('email.admin.enabled') }}</th><th class="px-5 py-3">{{ t('email.admin.action') }}</th></tr></thead><tbody>
          <tr v-for="provider in providers" :key="provider.id" class="border-t border-gray-100 dark:border-dark-700"><td class="min-w-[280px] whitespace-nowrap px-5 py-3 font-medium text-gray-900 dark:text-white"><div class="flex min-w-max flex-nowrap items-center gap-2 whitespace-nowrap"><span class="whitespace-nowrap">{{ provider.name }}</span><a v-if="providerPortalUrl(provider)" :href="providerPortalUrl(provider)" target="_blank" rel="noopener noreferrer" class="inline-flex min-h-8 shrink-0 items-center gap-1 whitespace-nowrap rounded-md px-2 text-xs text-primary-600 hover:bg-primary-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 dark:text-primary-300 dark:hover:bg-primary-950/30" :title="t('email.admin.openProvider')"><Icon name="externalLink" size="xs" aria-hidden="true" /><span>{{ t('email.admin.portal') }}</span></a></div></td><td class="min-w-[280px] whitespace-nowrap px-5 py-3"><input v-model="provider.base_url" class="input min-w-60" :aria-label="`${t('email.admin.baseUrl')} - ${provider.name}`" /></td><td class="min-w-[280px] whitespace-nowrap px-5 py-3">
  ...
  <span v-else class="badge badge-success">{{ t('email.admin.noCredentialRequired') }}</span>
</td><td class="min-w-[120px] whitespace-nowrap px-5 py-3"><span class="badge" :class="provider.health_status === 'healthy' ? 'badge-success' : 'badge-warning'">{{ t(`email.admin.status.${provider.health_status}`, provider.health_status) }}</span></td><td class="min-w-[150px] whitespace-nowrap px-5 py-3"><div class="flex min-w-max flex-nowrap items-center gap-2 whitespace-nowrap"><Toggle v-model="provider.enabled" :label="t('email.admin.providerEnabled', { name: provider.name })" :disabled="isProviderSaving(provider.id)" @update:model-value="saveProvider(provider)" /><span class="text-xs font-medium text-gray-600 dark:text-gray-300">{{ provider.enabled ? t('common.enabled') : t('common.disabled') }}</span></div></td><td class="min-w-[200px] whitespace-nowrap px-5 py-3"><div class="flex min-w-max flex-nowrap gap-2 whitespace-nowrap"><button type="button" class="btn btn-secondary btn-sm shrink-0 whitespace-nowrap" :disabled="isProviderSaving(provider.id)" :aria-busy="isProviderSaving(provider.id)" @click="saveProvider(provider)">{{ isProviderSaving(provider.id) ? t('email.admin.saving') : t('email.admin.save') }}</button><button type="button" class="btn btn-secondary btn-sm shrink-0 whitespace-nowrap" :disabled="testingProviderId === provider.id || isProviderSaving(provider.id)" :aria-busy="testingProviderId === provider.id" :title="t('email.admin.testRequestNotice')" @click="testProvider(provider)">{{ testingProviderId === provider.id ? t('email.admin.testing') : t('email.admin.testConnection') }}</button></div></td></tr>
        </tbody></table></div>
      </section>
      ...
      <section class="card min-w-0 overflow-hidden">
        ...
        <div class="max-w-full overflow-x-auto overscroll-x-contain focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary-500" tabindex="0" :aria-label="t('email.admin.channels')"><table class="w-full min-w-[2800px] text-left text-sm"><thead class="whitespace-nowrap bg-gray-50 text-xs uppercase text-gray-500 dark:bg-dark-800"><tr><th class="px-5 py-3">{{ t('email.admin.channels') }}</th><th class="px-5 py-3">{{ t('email.admin.providers') }}</th><th class="px-5 py-3">{{ t('email.admin.salePrice') }}</th><th class="px-5 py-3">{{ t('email.admin.baseMarkup') }}</th><th class="px-5 py-3">{{ t('email.admin.fixedMarkup') }}</th><th class="px-5 py-3">{{ t('email.admin.minimumProfit') }}</th><th class="px-5 py-3">{{ t('email.admin.refundPolicy') }}</th><th class="px-5 py-3">{{ t('email.admin.capturePolicy') }}</th><th class="px-5 py-3">{{ t('email.admin.orderTtl') }}</th><th class="px-5 py-3">{{ t('email.admin.maxRequests') }}</th><th class="px-5 py-3">{{ t('email.admin.pollingBackoff') }}</th><th class="px-5 py-3">{{ t('email.admin.visible') }}</th><th class="px-5 py-3">{{ t('email.admin.health') }}</th><th class="px-5 py-3">{{ t('email.admin.enabled') }}</th><th class="px-5 py-3">{{ t('email.admin.action') }}</th></tr></thead><tbody>
          <tr v-for="channel in channels" :key="channel.id" class="border-t border-gray-100 dark:border-dark-700"><td class="min-w-[240px] whitespace-nowrap px-5 py-3 font-medium text-gray-900 dark:text-white"><span>{{ channel.public_name }}</span> <span class="font-mono text-xs text-gray-500">{{ channel.code }}</span></td><td class="min-w-[140px] whitespace-nowrap px-5 py-3">{{ channel.provider_name }}</td><td class="whitespace-nowrap px-5 py-3"><input v-model.number="channel.sale_price" type="number" min="0" step="0.01" class="input w-28" :aria-label="`${t('email.admin.salePrice')} - ${channel.public_name}`" /></td><td class="whitespace-nowrap px-5 py-3"><input v-model.number="channel.base_markup" type="number" min="0" step="0.01" class="input w-24" :aria-label="`${t('email.admin.baseMarkup')} - ${channel.public_name}`" /></td><td class="whitespace-nowrap px-5 py-3"><input v-model.number="channel.fixed_markup" type="number" min="0" step="0.01" class="input w-24" :aria-label="`${t('email.admin.fixedMarkup')} - ${channel.public_name}`" /></td><td class="whitespace-nowrap px-5 py-3"><input v-model.number="channel.minimum_profit" type="number" min="0" step="0.01" class="input w-24" :aria-label="`${t('email.admin.minimumProfit')} - ${channel.public_name}`" /></td><td class="whitespace-nowrap px-5 py-3"><select v-model="channel.refund_policy" class="input min-w-56" :aria-label="`${t('email.admin.refundPolicy')} - ${channel.public_name}`"><option value="refund_if_no_message">{{ t('email.admin.refundPolicies.refund_if_no_message') }}</option><option value="no_refund_after_inbox_delivery">{{ t('email.admin.refundPolicies.no_refund_after_inbox_delivery') }}</option><option value="manual_review">{{ t('email.admin.refundPolicies.manual_review') }}</option></select></td><td class="whitespace-nowrap px-5 py-3"><select v-model="channel.capture_policy" class="input min-w-56" :aria-label="`${t('email.admin.capturePolicy')} - ${channel.public_name}`"><option value="on_target_email_received">{{ t('email.admin.capturePolicies.on_target_email_received') }}</option><option value="on_verification_extracted">{{ t('email.admin.capturePolicies.on_verification_extracted') }}</option></select></td><td class="whitespace-nowrap px-5 py-3"><input v-model.number="channel.order_ttl_seconds" type="number" min="60" max="86400" class="input w-28" :aria-label="`${t('email.admin.orderTtl')} - ${channel.public_name}`" /></td><td class="whitespace-nowrap px-5 py-3"><input v-model.number="channel.max_provider_requests_per_order" type="number" min="1" max="10000" class="input w-24" :aria-label="`${t('email.admin.maxRequests')} - ${channel.public_name}`" /></td><td class="whitespace-nowrap px-5 py-3"><input v-model="channelBackoffDrafts[channel.id]" class="input min-w-52 font-mono text-xs" :placeholder="t('email.admin.backoffPlaceholder')" :aria-label="`${t('email.admin.pollingBackoff')} - ${channel.public_name}`" /></td><td class="w-24 whitespace-nowrap px-5 py-3 text-center"><Toggle v-model="channel.visible" :label="t('email.admin.channelVisible', { name: channel.public_name })" :disabled="isChannelSaving(channel.id)" @update:model-value="saveChannel(channel)" /></td><td class="w-24 whitespace-nowrap px-5 py-3 text-center"><Toggle v-model="channel.healthy" :label="t('email.admin.channelHealthy', { name: channel.public_name })" :disabled="isChannelSaving(channel.id)" @update:model-value="saveChannel(channel)" /></td><td class="min-w-[150px] whitespace-nowrap px-5 py-3"><div class="flex min-w-max flex-nowrap items-center gap-2 whitespace-nowrap"><Toggle v-model="channel.enabled" :label="t('email.admin.channelEnabled', { name: channel.public_name })" :disabled="isChannelSaving(channel.id)" @update:model-value="saveChannel(channel)" /><span class="text-xs font-medium text-gray-600 dark:text-gray-300">{{ channel.enabled ? t('common.enabled') : t('common.disabled') }}</span></div></td><td class="min-w-[100px] whitespace-nowrap px-5 py-3"><button type="button" class="btn btn-secondary btn-sm whitespace-nowrap" :disabled="isChannelSaving(channel.id)" :aria-busy="isChannelSaving(channel.id)" @click="saveChannel(channel)">{{ isChannelSaving(channel.id) ? t('email.admin.saving') : t('email.admin.save') }}</button></td></tr>
        </tbody></table></div>
      </section>
    </div>
  </AppLayout>

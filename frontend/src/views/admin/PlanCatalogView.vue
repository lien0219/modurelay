...
function openCreate(){ Object.assign(form, blank()); benefitsText.value=''; editing.value=null; dialog.value=true }
function openEdit(item: PlanCatalogItem){ Object.assign(form, { ...item, original_price:item.original_price ?? null }); benefitsText.value=item.benefits.join('\n'); editing.value=item.id; dialog.value=true }
async function save(){ saving.value=true; try { form.benefits=benefitsText.value.split('\n').map(v=>v.trim()).filter(Boolean); if(editing.value) await adminPlanCatalogAPI.update(editing.value, form); else await adminPlanCatalogAPI.create(form); dialog.value=false; await load() } finally { saving.value=false } }
...
<style scoped>.field{display:block;font-size:.875rem;font-weight:500;color:rgb(75 85 99)}.dark .field{color:rgb(209 213 219)}</style>

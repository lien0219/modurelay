...
production_image=${PRODUCTION_IMAGE:-"ghcr.io/${owner,,}/modurelay"}
registries=("$production_image")
if [[ ${SIMPLE_RELEASE:-false} != true && ${DOCKERHUB_USERNAME:-skip} != skip ]]; then
  registries+=("${DOCKERHUB_USERNAME}/modurelay")
fi
arches=(amd64 arm64)
...
for arch in "${arches[@]}"; do
  ...
  if [[ ${DRY_RUN:-false} == true ]]; then
    args+=(--output "type=oci,dest=$RUNNER_TEMP/modurelay-$arch.oci.tar")
  else
    args+=(--push)
  fi

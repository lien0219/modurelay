...
+ [新增] ModuRelay 画布连接支持从当前账户的有效 Key 中选择复用，或手动选择可用分组后创建专用 Key；导航新增个人中心，可查看账户余额、并发、可用分组和掩码 Key。
+ [修复] ModuRelay 内嵌画布拉取第三方模型列表时改走登录保护的同源接口，避免浏览器跨域预检失败，并保留手动添加模型的恢复路径。
+ [修复] ModuRelay 内嵌画布的图片、文本、视频、音频、自定义调用脚本和生成媒体下载统一走受控同源代理，避免生成阶段仍被浏览器 CORS 拦截。
+ [调整] ModuRelay 与无限画布之间的转场改为无文字、无进度条的半透明磨砂玻璃双开门效果，页面在门下保持可见并随开合逐渐模糊或清晰。

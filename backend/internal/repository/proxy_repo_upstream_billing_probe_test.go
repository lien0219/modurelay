...
func TestProxyUpdateInvalidatesBoundProbeSnapshotsAndEnqueuesOutboxAtomically(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)UPDATE accounts.*- 'upstream_billing_probe'.*- 'upstream_billing_auto_unschedulable'.*- 'ollama_cloud_usage_snapshot'.*- 'opencode_go_usage_snapshot'.*type = 'apikey'.*extra \? 'upstream_billing_probe'.*extra ->> 'upstream_billing_auto_unschedulable'.*platform IN \('openai', 'anthropic', 'kimi', 'zhipu', 'deepseek'[^)]*\).*extra \? 'ollama_cloud_usage_snapshot'.*opencode_go_usage_snapshot.*RETURNING id`).
		WithArgs(int64(9)).

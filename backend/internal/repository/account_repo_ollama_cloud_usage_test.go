...
func TestUpdateCredentialsPlainCNAPIKeyAccountCleanupStaysSemanticallyEquivalent(t *testing.T) {
	...
	require.Contains(t, query,
		"THEN COALESCE(extra, '{}'::jsonb) - 'upstream_billing_probe' - 'upstream_billing_auto_unschedulable' - 'ollama_cloud_usage_session' - 'ollama_cloud_usage_auto_refresh' - 'ollama_cloud_usage_snapshot'")

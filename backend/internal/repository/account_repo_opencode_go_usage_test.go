...
func openCodeGoMergeMockColumns() []string {
	return []string{
		...
		"schedulable", "probe_enabled", "probe_rate_sync", "probe_snapshot",
		"auto_unschedulable", "ollama_session", "ollama_auto", "ollama_snapshot",
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
				...
				WillReturnRows(sqlmock.NewRows(openCodeGoMergeMockColumns()).
					AddRow(true, tt.groupUnchanged, tt.proxyUnchanged, true, nil, nil, nil, nil, nil, tt.databaseAuto, tt.databaseSnapshot, tt.groupUnchanged, tt.databaseAuto, tt.databaseSnapshot))

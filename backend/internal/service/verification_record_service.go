...
const verificationRecordsCTE = `WITH records AS (
	SELECT
		...
		CASE WHEN o.status IN ('active', 'completed', 'expired') THEN o.sale_price_snapshot::float8 ELSE 0::float8 END AS captured_amount,
		CASE WHEN o.status = 'failed' THEN o.sale_price_snapshot::float8 ELSE 0::float8 END AS released_amount,
		CASE WHEN o.status IN ('refunded', 'cancelled') OR o.refund_status = 'approved' THEN o.sale_price_snapshot::float8 ELSE 0::float8 END AS refunded_amount,

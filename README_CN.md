...
ModuRelay 是基于 [ModuRelay](https://github.com/Wei-Shaw/modurelay) 独立维护的衍生开源项目。

- 不属于 ModuRelay 官方项目
- 未获得上游官方背书
- 上游仓库：[Wei-Shaw/modurelay](https://github.com/Wei-Shaw/modurelay)
...
| 智能账号调度 | 按负载与策略选择账号 |
| 粘性会话 | 在配置启用时尽量将相关请求落到同一账号 |
| 用量统计 | 记录 Token 与请求统计 |
| 计费控制 | 支持倍率、余额等相关计费设置 |
| 并发控制 | 限制用户 / 分组并发 |
| RPM / 限流 | 应用 RPM 等相关限流策略 |
| 管理后台 | 基于 Vue 的运营控制台 |
| 支付系统 | 可选支付能力（见 `docs/PAYMENT.md`） |
...
- 仅可在符合当地法律法规的前提下使用
- 你自行负责所配置的账号、API Key 与凭据
- 上游账号稳定性与供应商可用性不作保证
- 本项目不提供任何 AI 供应商官方授权
- 部署与运营风险由使用者自行承担
- 禁止用于违法用途

以上为 ModuRelay 项目提示，不复述任何上游商业授权声明。

## 赞助商

本仓库不转载上游项目的赞助商广告、邀请码或推广链接。这些内容属于 ModuRelay，不代表 ModuRelay 的赞助关系。

上游赞助信息请查看 [ModuRelay 原仓库](https://github.com/Wei-Shaw/modurelay)。
...
- ModuRelay 与 ModuRelay 上游维护者不存在官方隶属关系
